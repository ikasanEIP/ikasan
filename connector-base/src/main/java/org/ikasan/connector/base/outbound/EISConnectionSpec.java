/*
 * $Id: EISConnectionSpec.java 16756 2009-04-22 12:35:57Z mitcje $
 * $URL: svn+ssh://svc-vcsp/architecture/ikasan/trunk/connector-base/src/main/java/org/ikasan/connector/base/outbound/EISConnectionSpec.java $
 * 
 * ====================================================================
 ...
 * 
 * This does not represent the physical EIS properties ie socket, port. This
 * is in the ra.xml and/or the ConnectionRequestInfo.
 * 
 * @author Ikasan Development Team
 */
public abstract class EISConnectionSpec 
    implements ConnectionSpec, EISConnectionProperties
{
    /** logger */
    ...
    public java.util.ResourceBundle loadSessionProps(String sessionPropertiesHandle)
    {
        ...
        try
        {
            ...
            //
            // make sure we dont have null or empty session props
            if(sessionProperties == null || sessionProperties.trim().length() == 0)
                throw new NamingException("Context SessionPropertiesHandle ["  //$NON-NLS-1$
                        + sessionPropertiesHandle + "] is is empty."); //$NON-NLS-1$

            return ResourceBundle.getBundle(sessionProperties, 
        ...
        catch(NamingException e)
        {
            ...
            try
            {
                ...
                //return ResourceBundle.getBundle(sessionPropertiesHandle, 
                  //      Locale.getDefault(), this.getClass().getClassLoader());
            }
            catch(MissingResourceException e1)
            {
                // Not found on the classpath, try the default sessionProperties name
                try
                {
                    return ResourceBundle.getBundle(DEFAULT_SESSION_PROPERTIES, 
                            Locale.getDefault(), this.getClass().getClassLoader());
                }
