/*
 * $Id: TCPSocket.java 16756 2009-04-22 12:35:57Z mitcje $
 * $URL: svn+ssh://svc-vcsp/architecture/ikasan/trunk/connector-base/src/main/java/org/ikasan/connector/base/socket/TCPSocket.java $
 * 
 * ====================================================================
...
public class TCPSocket extends java.net.Socket
{
    ...
    /**
     * Utility constructor that takes as parameters the target host and port
     * and uses the underlying defaults for response and connection timeouts.
     * 
     ...
     */
    public void send(byte[] data, byte startMarker, byte endMarker) 
        throws IOException
    {
        byte[] framedData = frameOutboundData(data, startMarker, endMarker);
        if (logSocketTraffic)
            logger.info("Sending [" + Arrays.toString(framedData) + "]...");  //$NON-NLS-1$//$NON-NLS-2$
        try
        {
    ...
    public void send(String data, byte startMarker, byte endMarker) throws IOException
    {
        ...
        byte[] framedData = 
            frameOutboundData(this.stringToByteArray(data), startMarker, endMarker);
     ...
     * 
     * @param startMarker The <code>byte</code> marker indicating the start of a message.
     * @param endMarker The <code>byte</code> marker indicating the end of a message.
     * @return A <code>byte[]</code> representation of a single message received.
     ...
     */
    public byte[] recv(final byte startMarker, final byte endMarker) 
        throws SocketTimeoutException, SocketException, IOException
    {
        ...
        try
        {
            ...
            do
            {
                if((bytesRead = this.bins.read(singleByte)) < 0)
                    throw new SocketException ("Connection reset by peer; EOF received!"); //$NON-NLS-1$
        ...
        catch (SocketTimeoutException e)
        {
            if (badBuffer.size() > 0)
            {
                ...
                logger.warn("Unexpected data read from socket ["  //$NON-NLS-1$
                    ...
                    + "] before timeout and will be ignored. Unexpected data ["  //$NON-NLS-1$
        ...
        // ---- However, this may not be a great idea as this is only a marker-to-marker solution ----
        
        
        // If any unexpected data was found, ignore it but issue a warning
        if (badBuffer.size() > 0)
        {
            logger.warn("Unexpected data read from socket ["  + super.getInetAddress() + ':' 
                + super.getPort() + "] which will be ignored. Unexpected data ["  
                + badBuffer.toString() + "].");
        }

        // If we haven't timed out and startMarker was found, read the expected data
        try
        {
            int bytesRead = 0;
            int timeouts = ((this.getSoTimeout() == 0) && 
                    (this.getSoTimeout() >= MAXIMUM_RESPONSE_TIMEOUT)) ? 0 : 
                        (MAXIMUM_RESPONSE_TIMEOUT / this.getSoTimeout());
                ...
                try
                {
                    if((bytesRead = this.bins.read(singleByte)) < 0)
                    {
                        ...
                        if(msgBuffer.size() > 0)
                        {
                            ...
                            if (logSocketTraffic)
                            sb.append(" [").append(Arrays.toString(msgBuffer.toByteArray())).append(']'); //$NON-NLS-1$
                        }
                        throw new SocketException (sb.toString());
                    }
                    if (bytesRead > 0 && singleByte[0] != endMarker)
                        msgBuffer.write(singleByte);
                }
                // Timeout on partial data read - if the timeout is very small, this could be due to
                // network traffic. So need an intelligent way of ignoring a number of these.
                catch (SocketTimeoutException e)
                {
                    if (timeouts == 0)
                        throw e;
            ...
            if (logSocketTraffic)
                logger.info("Received [" + Arrays.toString(msgBuffer.toByteArray()) + "]");  //$NON-NLS-1$//$NON-NLS-2$
            
            // keep track of the last time we successfully read data off the line
            this.recvActivity = java.util.Calendar.getInstance().getTimeInMillis();
            return msgBuffer.toByteArray();
        }
        catch (SocketTimeoutException e)
        {
            // Socket timed-out without receiving a legitimate endMarker - hence not valid message.
    ...
    /**
     * This method is currently more a place-holder for more elaborate conversion
     * of <code>String</code>s to <code>byte[]</code>
     * 
    ...
    private byte[] stringToByteArray(String string)
    {
        ...
        // CoderResult encodingResult = encoder.encode(charBuffer, buffer, true /*no more input*/);
        // return buffer.array();
