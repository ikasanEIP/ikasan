/*
 * $Id: EISConnectionFactoryImpl.java 16756 2009-04-22 12:35:57Z mitcje $
 * $URL: svn+ssh://svc-vcsp/architecture/ikasan/trunk/connector-base/src/main/java/org/ikasan/connector/base/outbound/EISConnectionFactoryImpl.java $
 * 
 * ====================================================================
...
/**
 * This class implements the ConnectionFactory for the EIS resource
 * adapter. Each type of adapter derived from this class should implement
 * getConnection() for the adapter specifics.
 * Clients get an instance of this class by doing a JNDI lookup.
 * 
 * @author Ikasan Development Team
 */  
public abstract class EISConnectionFactoryImpl 
    implements EISConnectionFactory, Serializable, javax.resource.Referenceable
{
    /** SerialVersion */
     ...
     * Actual connection must be created by the derived class
     */
    public abstract Connection getConnection(ConnectionSpec spec)
     ...
     * @throws ResourceException 
     */
    protected final Connection allocateConnection(ConnectionRequestInfo connRequestInfo)
        throws ResourceException 
    {
        logger.debug("allocateConnection(connRequestInfo)..."); //$NON-NLS-1$
        
        Connection connection = (Connection)this.connectionManager.
            allocateConnection(this.managedConnectionFactory, connRequestInfo); 
     ...
     * application server can save references to this object in its JNDI
     * store, rather than serializing the object itself. The spec
     * requires both reference and serialization methods to be supported.
     * Of course, the serialization method is automatic; we just say 
     * 'implements serializable'. Note that setReference is called by
     * the application server, not by any part of the resource adapter;
     * we don't know (or care) what it passes as an argument, so long
     * as we can retrieve it again on demand.
     */  
