...
mvn archetype:generate     
    \-DarchetypeGroupId=org.ikasan     
    \-DarchetypeArtifactId=ikasan-standalone-db-jms-im-maven-plugin 
    \-DarchetypeVersion=3.2.4-SNAPSHOT    
    \-DgroupId=com.sample     
...
This will build and create a zip binary (specifically ```distribution/target/db-jms-im-distribution-${project.version}-dist.zip```) containing all the required artefacts for the deployment of your integration module.

### Deploy
Copy the created zip image from to the required destination, unzip and navigate into the unzipped directory.
```xslt
 unzip db-jms-im-distribution-${project.version}-dist.zip
 cd db-jms-im-distribution-${project.version}
```
...
``` 
 mvn archetype:generate     -DarchetypeGroupId=org.ikasan     -DarchetypeArtifactId=ikasan-standalone-filesystem-im-maven-plugin -DarchetypeVersion=3.2.4-SNAPSHOT
