...
public class ${className} implements Broker<#if parameterizedType??><<#list parameterizedType.typeParameters as typeParam><#if typeParam.name=="SEND">${typeParam.parameterClass}</#if></#list>, <#list parameterizedType.typeParameters as typeParam><#if typeParam.name=="RECEIVE">${typeParam.parameterClass}</#if></#list>></#if><#if isConfigured> , ConfiguredResource<${configurationMetaData.configurationClassName}> </#if> {
    ...
    @Override
    public <#if parameterizedType??><#list parameterizedType.typeParameters as typeParam><#if typeParam.name=="RECEIVE">${typeParam.parameterClass}</#if></#list><#else>Object</#if> invoke(<#if parameterizedType??><#list parameterizedType.typeParameters as typeParam><#if typeParam.name=="SEND">${typeParam.parameterClass}</#if></#list><#else>Object</#if> payload) {
