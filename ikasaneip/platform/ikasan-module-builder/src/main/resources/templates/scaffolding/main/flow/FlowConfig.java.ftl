...
<#macro routeMethod routerName key component>
    ...
    */
    private Route route${routerName?replace(" ", "")?replace(",", "")?cap_first}${key?replace(" ", "")?replace(",", "")?cap_first}(RouteBuilder routeBuilder) {
        return routeBuilder
        <#if instanceOf(component, "org.ikasan.module.builder.model.module.SingleTransition")>
...
<#macro addSingleTransitionComponentToBuilder component primaryRoute>
    ...
    <#if instanceOf(component, "org.ikasan.module.builder.model.module.ConsumerComponent")>
        .consumer("${component.name}", componentFactory.get${component.name?replace(" ", "")?replace(",", "")?cap_first}())
    </#if>
    <#if instanceOf(component, "org.ikasan.module.builder.model.module.ProducerComponent")>
        .producer("${component.name}", componentFactory.get${component.name?replace(" ", "")?replace(",", "")?cap_first}())<#if primaryRoute == "false">;</#if>
        <#if primaryRoute == "true">
        .build();
        </#if>
    </#if>
    <#if instanceOf(component, "org.ikasan.module.builder.model.module.BrokerComponent")>
        .broker("${component.name}", componentFactory.get${component.name?replace(" ", "")?replace(",", "")?cap_first}())
    </#if>
    <#if instanceOf(component, "org.ikasan.module.builder.model.module.ConverterComponent")>
        .converter("${component.name}", componentFactory.get${component.name?replace(" ", "")?replace(",", "")?cap_first}())
    </#if>
    <#if instanceOf(component, "org.ikasan.module.builder.model.module.SequencerComponent")>
        .sequencer("${component.name}", componentFactory.get${component.name?replace(" ", "")?replace(",", "")?cap_first}())
    </#if>
    <#if instanceOf(component, "org.ikasan.module.builder.model.module.SplitterComponent")>
        .splitter("${component.name}", componentFactory.get${component.name?replace(" ", "")?replace(",", "")?cap_first}())
    </#if>
    <#if instanceOf(component, "org.ikasan.module.builder.model.module.TranslatorComponent")>
        .translator("${component.name}", componentFactory.get${component.name?replace(" ", "")?replace(",", "")?cap_first}())
    </#if>
    <#if instanceOf(component, "org.ikasan.module.builder.model.module.FilterComponent")>
        .filter("${component.name}", componentFactory.get${component.name?replace(" ", "")?replace(",", "")?cap_first}())
    </#if>
    <#if component.transition??>
...
<#macro addMultiTransitionComponentToBuilder component>
    ...
    <#if instanceOf(component, "org.ikasan.module.builder.model.module.MultiRecipientRouterComponent")>
        .multiRecipientRouter("${component.name}", componentFactory.get${component.name?replace(" ", "")?replace(",", "")?cap_first}())
        <#list component.transitions as key, value >
            .when("${key}", route${component.name?replace(" ", "")?replace(",", "")?cap_first}${key?replace(" ", "")?replace(",", "")?cap_first}(builderFactory.getRouteBuilder()))
        </#list>
        .build();
    </#if>
    <#if instanceOf(component, "org.ikasan.module.builder.model.module.SingleRecipientRouterComponent")>
        .singleRecipientRouter("${component.name}", componentFactory.get${component.name?replace(" ", "")?replace(",", "")?cap_first}())
        <#list component.transitions as key, value >
            .when("${key}", route${component.name?replace(" ", "")?replace(",", "")?cap_first}${key?replace(" ", "")?replace(",", "")?cap_first}(builderFactory.getRouteBuilder()))
        </#list>
        .build();
...
@Configuration
public class ${name?replace(" ", "")?replace(",", "")?cap_first}Config
{
    @Value("${"$"}{module.name}")
    ...
    */
    @Bean(name = "${name?replace(" ", "")?replace(",", "")?uncap_first}")
    public Flow ${name?replace(" ", "")?replace(",", "")?cap_first}()
    {
