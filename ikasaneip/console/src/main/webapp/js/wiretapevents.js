 ...
 * JQuery code
 * 
 * Isn't enabled unless the document is 'ready' (e.g. the DOM is fully
 * loaded)
 */
$(document).ready( function() {
	...
	// jquery date picker assistance      
		...
		// If the untilDateString changes then set the untilTimeString to be a
		// default value
		$("#untilDateString").change( function() {
			$("#untilTimeString").val('00:00:00');
		});

		// Shows/hides the searchFields div on clicking the link with an ID of
		// "showHideSearchForm"
		 ...
		 * "toggleSearchMode", can't simply use jquery toggle() between the two
		 * because the show/hide searchFields sets the visibility on both
		 * checkbox sections.
		 */
